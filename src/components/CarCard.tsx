...
export default function CarCard({ car, horizontal = false }: { car: Car, horizontal?: boolean }) {
  ...
  return (
    <Link 
      ...
      className={
        horizontal
          ? "group flex flex-row items-stretch rounded-2xl border border-gray-200 shadow-sm hover:shadow-xl transition-all duration-300 p-4 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 hover:-translate-y-1 hover:scale-[1.01] min-h-[180px] max-h-[220px]"
          : "group block rounded-2xl border border-gray-200 shadow-sm hover:shadow-xl transition-all duration-300 p-4 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transform hover:-translate-y-1 hover:scale-[1.02]"
      }
      aria-label={`${car.title} - ${price}`}
    >
      <div className={horizontal ? "relative w-56 min-w-[180px] max-w-[220px] h-36 overflow-hidden rounded-xl flex-shrink-0" : "relative aspect-[4/3] w-full overflow-hidden rounded-xl"}>
        {img && getImageUrl(img, 800) ? (
          <Image
            src={getImageUrl(img, 800)!}
            alt={`${car.title} Bild`}
            fill
            className={horizontal ? "object-cover" : "object-cover group-hover:scale-110 transition-transform duration-500"}
            sizes={horizontal ? "220px" : "(max-width: 768px) 100vw, 33vw"}
            priority={false}
          />
        ) : (
      ...
      <div className={horizontal ? "flex-1 flex flex-col justify-between pl-8 py-2 min-w-0" : "mt-4 space-y-3"}>
        {/* Title */}
        ...
        <div className={horizontal ? "grid grid-cols-4 gap-3 text-sm" : "grid grid-cols-2 gap-3 text-sm"}>
          {/* Baujahr */}
        ...
        <div className={horizontal ? "pt-2 mt-auto" : "pt-2"}>
